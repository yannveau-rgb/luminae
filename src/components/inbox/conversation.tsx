...
export function ConversationView({ conversationId, agent }: { conversationId: string; agent: Agent }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden">
      ...
      {/* ── Tiroir Latéral Droit : Contexte Client & Intelligence Haute Définition (Slide-over Mobile & Desktop) ────────────── */}
      {showDrawer && (
        <>
          {/* Backdrop mobile & tablette */}
          <div
            className="fixed inset-0 z-40 bg-ink-950/40 backdrop-blur-xs xl:hidden"
            onClick={() => setShowDrawer(false)}
            aria-hidden
          />
          <aside className="fixed inset-y-0 right-0 z-50 w-[310px] max-w-[85vw] flex flex-col border-l border-mist-300 bg-white p-4 shadow-2xl xl:static xl:shadow-none xl:z-auto xl:bg-mist-50/50 overflow-y-auto space-y-3.5 animate-slide-left">
            {/* Header du panneau */}
